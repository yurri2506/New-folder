...
class Menu:
    ...
    def update(self):
        self.draw()
        if ui.button(self.surface, 500, "Bắt đầu", click_sound=self.click_sound):
            return "game"
       
        if ui.button(self.surface, 500+BUTTONS_SIZES[1]*1.5, "Kết thúc", click_sound=self.click_sound):
            pygame.quit()
            sys.exit()
